...
// Defined in main.cpp - needed for SDL_WarpMouseInWindow()
...
int process_game(void)
{
    ...
    if (level_completed())
    {
        ...
        game.background.set_movement_type(BOUNCE);
        // SDL2: SDL_WarpMouse() -> SDL_WarpMouseInWindow(g_window, x, y)
        SDL_WarpMouseInWindow(g_window,
            game.graphics.gl_to_res(next_level_menu.get_button_x_pos(1),game.config.mouse_resolution_x),
            game.config.mouse_resolution_y - game.graphics.gl_to_res(next_level_menu.get_button_y_pos(1),game.config.mouse_resolution_y));
    }
    ...
    if ((game_o.current_level >=  4) && (!game_o.projectile[1].active) && (random(game_o.powerup[7].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
    if ((game_o.current_level >=  7) && (!game_o.projectile[2].active) && (random(game_o.powerup[7].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
    if ((game_o.current_level >= 11) && (!game_o.projectile[3].active) && (random(game_o.powerup[7].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
    if ((game_o.current_level >= 15) && (!game_o.projectile[4].active) && (random(game_o.powerup[7].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
    if ((game_o.current_level >= 19) && (!game_o.projectile[5].active) && (random(game_o.powerup[7].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon

    if ((game_o.current_level >=  7) && (!game_o.shield[0].active) && (random(game_o.powerup[3].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
    if ((game_o.current_level >= 11) && (!game_o.shield[1].active) && (random(game_o.powerup[3].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
    if ((game_o.current_level >= 15) && (!game_o.shield[2].active) && (random(game_o.powerup[3].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
    if ((game_o.current_level >= 19) && (!game_o.shield[3].active) && (random(game_o.powerup[3].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
    if ((game_o.current_level >= 23) && (!game_o.shield[4].active) && (random(game_o.powerup[3].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield

    if ((game_o.current_level >= 15) && (!game_o.thruster[0].active) && (random(game_o.powerup[5].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
    if ((game_o.current_level >= 19) && (!game_o.thruster[1].active) && (random(game_o.powerup[5].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
    if ((game_o.current_level >= 23) && (!game_o.thruster[2].active) && (random(game_o.powerup[5].spawn_rate) == 5)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
    game_o.npc_spawn_rate_count++;
    game_o.npc_spawn_rate_count += random_dec()/4;
    if (game_o.npc_spawn_rate_count >= game_o.npc_spawn_rate)
    {
        game_o.npc_spawn_rate_count = 0;
        if (game_o.level[game_o.current_level].BOSS && (game_o.level_spawened == 0))
        {
            spawn_npc(1.0f,random_GLcoord(),game_o.level[game_o.current_level].NPC_type,0,0.0f,0.0f);
            game_o.level_spawened += 1;
        }
        if ((!game_o.level[game_o.current_level].BOSS) && (!game_o.level_end_time) && (game_o.wave_spawnable)) spawn_wave();
    }
    ...
    if ((game_o.level_end_time) && (game_o.level_end_phase == 0)) //level end phase
    {
        ...
        {
            if ((game_o.level[game_o.current_level].BOSS) && (game_o.level[game_o.current_level + 1].locked))//if its first time to kill a boss spawn some powerups!!
            {
                if (game_o.current_level == 3)
                {
                ...
                if (game_o.current_level == 7)
                {
                    spawn_powerup(1.0f,random_GLcoord(),1);//spawn health power-up
                    spawn_powerup(1.0f,random_GLcoord(),3);//spawn shield new power-up
                    spawn_powerup(1.0f,random_GLcoord(),7);//new weapon
                }
                if (game_o.current_level == 11)
                {
                    spawn_powerup(1.0f,random_GLcoord(),1);//spawn health power-up
                    spawn_powerup(1.0f,random_GLcoord(),3);//spawn shield new power-up
                    spawn_powerup(1.0f,random_GLcoord(),7);//new weapon
                }
                if (game_o.current_level == 15)
                {
                    spawn_powerup(1.0f,random_GLcoord(),1);//spawn health power-up
            ...
            else
            {
                ...
                if ((game_o.current_level >=  4) && (!game_o.projectile[1].active)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
                if ((game_o.current_level >=  7) && (!game_o.projectile[2].active)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
                if ((game_o.current_level >= 11) && (!game_o.projectile[3].active)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
                if ((game_o.current_level >= 15) && (!game_o.projectile[4].active)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon
                if ((game_o.current_level >= 19) && (!game_o.projectile[5].active)) spawn_powerup(1.0f,random_GLcoord(),7); // spawn new weapon

                if ((game_o.current_level >=  7) && (!game_o.shield[0].active)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
                if ((game_o.current_level >= 11) && (!game_o.shield[1].active)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
                if ((game_o.current_level >= 15) && (!game_o.shield[2].active)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
                if ((game_o.current_level >= 19) && (!game_o.shield[3].active)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield
                if ((game_o.current_level >= 23) && (!game_o.shield[4].active)) spawn_powerup(1.0f,random_GLcoord(),3); // spawn new shield

                if ((game_o.current_level >= 15) && (!game_o.thruster[0].active)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
                if ((game_o.current_level >= 19) && (!game_o.thruster[1].active)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
                if ((game_o.current_level >= 23) && (!game_o.thruster[2].active)) spawn_powerup(1.0f,random_GLcoord(),5); // spawn new thruster
...
//-----------------------------------------------------------------------------------------------------------------------
    ...
    for (int count =0;count < MAX_BULLETS;count++) //player bullets
    {
        if (game_o.player.bullet[count].active)
        {
            ...
            if (game_o.player.bullet[count].warhead ==  0) texture.projectile_000.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  1) texture.projectile_001.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  2) texture.projectile_002.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  3) texture.projectile_003.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  4) texture.projectile_004.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  5) texture.projectile_005.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  6) texture.projectile_006.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  7) texture.projectile_007.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  8) texture.projectile_008.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead ==  9) texture.projectile_009.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 10) texture.projectile_010.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 11) texture.projectile_011.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 12) texture.projectile_012.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 13) texture.projectile_013.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 14) texture.projectile_014.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 15) texture.projectile_015.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 16) texture.projectile_016.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 17) texture.projectile_017.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 18) texture.projectile_018.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 19) texture.projectile_019.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 20) texture.projectile_020.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 21) texture.projectile_021.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 22) texture.projectile_022.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 23) texture.projectile_023.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 24) texture.projectile_024.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 25) texture.projectile_025.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 26) texture.projectile_026.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 27) texture.projectile_027.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 28) texture.projectile_028.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 29) texture.projectile_029.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 30) texture.projectile_030.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 31) texture.projectile_031.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 32) texture.projectile_032.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 33) texture.projectile_033.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
            if (game_o.player.bullet[count].warhead == 34) texture.projectile_034.draw(true,game_o.player.bullet[count].x_pos,game_o.player.bullet[count].y_pos,z_pos,game_o.player.bullet[count].width,game_o.player.bullet[count].height,game_o.player.bullet[count].angle);
        }
    }
    draw_supportships_bullets();
   ...
   for (int count = MAX_WEXPS;count >=1;count--)  // weapon experience
   {
    ...
    if (game_o.player.health > 0.0f) texture.health_bar.draw (false,-0.6f   +((game_o.player.health/game_o.player.max_health)/10), 0.9375f, 0.001f,((game_o.player.health/game_o.player.max_health)/5), 0.075f); //player health bar
    if (game_o.player.shield > 0.0f) texture.shield_bar.draw (false,-0.375f +((game_o.player.shield/game_o.player.max_shield)/10), 0.9375f, 0.001f,((game_o.player.shield/game_o.player.max_shield)/5), 0.075f); //player shield bar
    ...
    if (game_o.player.front_weapon > -1)
    {
        ...
        if(game_o.projectile[game_o.player.front_weapon].image == texture.projectile_032.ref_number) texture.projectile_032.draw(false,-0.125f, 0.9375f, 0.001f, 0.075f, 0.075f); // front weapon picture next to the exp bar
        if(game_o.projectile[game_o.player.front_weapon].image == texture.projectile_033.ref_number) texture.projectile_033.draw(false,-0.125f, 0.9375f, 0.001f, 0.075f, 0.075f); // front weapon picture next to the exp bar
        if(game_o.projectile[game_o.player.front_weapon].image == texture.projectile_034.ref_number) texture.projectile_034.draw(false,-0.125f, 0.9375f, 0.001f, 0.075f, 0.075f); // front weapon picture next to the exp bar
    ...
    if (game_o.player.side_weapon > -1)
    {
        ...
        if(game_o.projectile[game_o.player.side_weapon].image == texture.projectile_032.ref_number) texture.projectile_032.draw(false, 0.175f, 0.9375f, 0.001f, 0.075f, 0.075f); // side weapon picture next to the exp bar
        if(game_o.projectile[game_o.player.side_weapon].image == texture.projectile_033.ref_number) texture.projectile_033.draw(false, 0.175f, 0.9375f, 0.001f, 0.075f, 0.075f); // side weapon picture next to the exp bar
        if(game_o.projectile[game_o.player.side_weapon].image == texture.projectile_034.ref_number) texture.projectile_034.draw(false, 0.175f, 0.9375f, 0.001f, 0.075f, 0.075f); // side weapon picture next to the exp bar

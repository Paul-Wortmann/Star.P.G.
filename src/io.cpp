...
bool events_init(void)
{
   ...
   {
      config_data.joystick_enabled        = true;
      game.io.joystick_sensitivity        = 6400;
      config_data.joystick_no_buttons     = SDL_JoystickNumButtons(joystick);
   }
   else
   {
...
bool events_process(void)
{
    ...
    //-------------------- Mouse events-------------------------------
       ...
       {
           game.io.mouse_x      = res_to_gl(game.event.motion.x,config_data.mouse_resolution_x);
           game.io.mouse_y      = res_to_gl(game.event.motion.y,config_data.mouse_resolution_y);
           game.io.mouse_xrel   = res_to_gl(game.event.motion.xrel,config_data.mouse_resolution_x);
           game.io.mouse_yrel   = res_to_gl(game.event.motion.yrel,config_data.mouse_resolution_y);
           game.io.mouse_y *= -1;
       }
    ...
    //-------------------------- joystick / gamepad events --------------------------------------------
       ...
       {
          if ((game.event.jaxis.value < (-1*(config_data.joystick_sensitivity))) || (game.event.jaxis.value > config_data.joystick_sensitivity))
          {
             if (game.event.jaxis.axis == 0)
             {
                if(game.event.jaxis.value < -(-1*(config_data.joystick_sensitivity)))
                {
                   game.io.joystick_left  = true;
                   game.io.joystick_right = false;
                }
                if(game.event.jaxis.value > config_data.joystick_sensitivity)
                {
                   game.io.joystick_left  = false;
                   game.io.joystick_right = true;
                }

...
void language_class::load(std::string file_name)
{
    ...
    int           temp_int_data;
    bool          temp_bool_data;
    std::wstring  temp_string_data;
    std::wstring  temp_string_key;
    std::wstring  temp_string_value;
    int           count;
    std::wstring  data_line;
    std::wfstream  script_file(file_name.c_str(),std::ios::in|std::ios::binary);
    if (script_file.is_open())
    {
        while ( script_file.good() )
        {
            getline(script_file,data_line);
            {
                ...
                {
                    temp_char_UTF32         = '#';
                    temp_string_key   = StringToWString("");
                    temp_string_value = StringToWString("");
                    count = 0;
                    ...
                    temp_float_data  = (float) _wtof(temp_string_value.c_str());
                    temp_int_data    = (int)   _wtoi(temp_string_value.c_str());
                    if (temp_int_data == 1) temp_bool_data = true;
                    else temp_bool_data = false;
                    if (temp_string_key == L"Main_Menu")
                    {
                        main_menu.set_menu_title(temp_string_data);
                        game.log.File_Write(L"Language data read ->",temp_string_data);
